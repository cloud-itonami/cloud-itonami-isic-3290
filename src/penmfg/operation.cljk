(ns penmfg.operation
  "WritingInstrumentOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (WritingInstrumentAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Writing Instrument Plant Operations Governor (:govern) and the
  ...
  each operation is auditable and checkpointed. A plant's lifecycle
  is advanced by MANY ops (production-batch logging / maintenance
  ...
  decision to a human plant supervisor/shipping approver. The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [penmfg.advisor :as advisor]
...
(defn build
  "Compiles a WritingInstrumentOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `penmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WritingInstrumentAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Writing Instrument Plant Operations Governor -- independent censor.
      (g/add-node :govern
